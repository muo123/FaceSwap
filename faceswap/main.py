...
originpath = 'extract_folder/'
transferpath = 'out_folder/'
...
class mywindow(QMainWindow,Ui_MainWindow): #这个窗口继承了用QtDesignner 绘制的窗口
    ...
    def start(self):
        ...
        video="C:/test/faceswap/data/test.mp4"
        image="C:/test/faceswap/data/1.jpg"
    ...
    def timerEvent(self, e):
        if self.step >= 100:
            ...
            return
        if self.step < 99 and self.step >= 70:
            self.step = self.step+0.16
...
class MyThread(QThread):  
    ...
    def run(self):
        if self.flag==1:
            # if FLAG==0 or FLAG==1:
            vedio_swap.main(VIDEO_PATH,TARGET_IMG,OUT_PATH,FLAG)
            # else:
            image_swap.ex_run(IMAGE_PATH,TARGET_IMG,OUT_PATH)
            self.flag=0
